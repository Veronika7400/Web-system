...
<%if(poruka == null){ %>
 <form action="${pageContext.servletContext.contextPath}/mvc/letovi/pregledSpremljenih" method="get">
  ...
  <label for="danDo">Dan do:</label>
  <input type="text" id="danDo" name="danDo" placeholder="<%=danDo%>">
  ...
  <button class="gumb"><a href=${pageContext.servletContext.contextPath}/mvc/letovi/pregledSpremljenih?odBroja=1&broj=<%=broj%>&icao=<%=icao%>&danOd=<%=danOd%>&danDo=<%=danDo%>>Početak</a></button></t>

<%  if(request.getAttribute("polasci") != null && !((List) request.getAttribute("polasci")).isEmpty()) {
	if(odBroja > 1) {
  %>
 <button class="gumb">
    <a href=${pageContext.servletContext.contextPath}/mvc/letovi/pregledSpremljenih?odBroja=<%= odBroja-broj %>&broj=<%=broj%>&icao=<%=icao%>&danOd=<%=danOd%>&danDo=<%=danDo%>>Prethodna stranica</a>
 <%
}
%>
</button></t>
...
<button class="gumb">
    <a href=${pageContext.servletContext.contextPath}/mvc/letovi/pregledSpremljenih?odBroja=<%= odBroja+broj %>&broj=<%=broj%>&icao=<%=icao%>&danOd=<%=danOd%>&danDo=<%=danDo%>>Sljedeća stranica</a>
